...
import type { Reservation, CreateReservationDto, UpdateReservationDto, ReservationFilters } from '../types';

export const reservationKeys = {
    all: ['reservations'] as const,
    lists: () => [...reservationKeys.all, 'list'] as const,
    list: (filters: ReservationFilters & { page?: number; limit?: number }) => [...reservationKeys.lists(), filters] as const,
    details: () => [...reservationKeys.all, 'detail'] as const,
    detail: (id: number) => [...reservationKeys.details(), id] as const,
    availableTables: (params: { date: string; time: string; duration: number; partySize: number }) => [...reservationKeys.all, 'available-tables', params] as const,
};

export function useReservations(filters: ReservationFilters & { page?: number; limit?: number } = {}) {
    return useQuery({
...
export function useUpdateReservation() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, data }: { id: number; data: UpdateReservationDto }) => reservationApi.update(id, data),
...
export function useAvailableTables(params: { date: string; time: string; duration: number; partySize: number }) {
    return useQuery({
